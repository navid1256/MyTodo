...
final class ProfileService
{
    ...
    public function deleteAvatar(string $avatarPath): void
    {
        ...
        $absolutePath = $rootPath . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $avatarPath);
    ...
    public function validateProfile(
        ...
        if (!in_array($avatarAction, ['unchanged', 'boring', 'upload'], true)) {
            $errors[] = 'Please choose a valid profile picture option.';
        }

        if ($avatarAction === 'boring' && (!ctype_digit($avatarChoiceValue) || (int) $avatarChoiceValue < 1 || (int) $avatarChoiceValue > 12)) {
            $errors[] = 'Please choose a valid avatar.';
        }

        $dob = trim($profileInput['date_of_birth'] ?? '');
        if ($dob !== '') {
    ...
    public function updateProfile(
        ...
        try {
            if ($avatarAction !== 'unchanged') {
                ...
                $profileData['avatar_url'] = $newAvatarPath;
            }
